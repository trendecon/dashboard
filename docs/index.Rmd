...
trendecon_all_sa <- read_csv("https://raw.githubusercontent.com/trendecon/data/master/data/ch/trendecon_sa.csv") %>%
  select(time,value) %>%
  ts_xts()
...
##### Latest News
- ###### *trendEcon on TV:*
  + Don't miss the TV interview on RSI LA1 at 22.25 on Friday, September 18th.
  Repetition on Saturday at 13:15.
  + Sehen Sie das Interview mit trendEcon im TV. Es wird am Freitag, dem 18. September um 22:25 Uhr auf RSI LA1 gesendet. Die Wiederholung wird am Samstag um 13:15 Uhr ebenfalls auf RSI LA1 ausgestrahlt.
...
data <- read_csv("https://raw.githubusercontent.com/trendecon/data/master/data/ch/trendecon_sa.csv") %>%
  select(time,value) %>%
...
### {}
```{r}
article_6 <- read_csv("https://raw.githubusercontent.com/trendecon/media-content/master/data_examples/article_6.csv") %>%
  ts_xts()
...
dm_social_sa <- social_sa %>%
  ts_frequency("month")
dm_social_sa_2016_2020 <- ts_data.frame(dm_social_sa["2016/2020-09-17"]) %>% 
  ts_xts()

ts_dygraphs(ts_c(article_6,`Cultural Event trendEcon Index`=dm_social_sa_2016_2020)) %>%
  ...
  dySeries("Cultural Event trendEcon Index", strokeWidth = 2, color="#E2D800") %>%
...
article_3 <- read_csv("https://raw.githubusercontent.com/trendecon/media-content/master/data_examples/article_3.csv") %>%
  ts_xts()
...
blog_ar_4 <- read_csv("https://raw.githubusercontent.com/trendecon/media-content/master/data_examples/article_4.csv") %>%
  ts_xts()
...
blog_ar_3_a <- read_csv("https://raw.githubusercontent.com/trendecon/media-content/master/data_examples/article_3_a.csv") %>%
  ts_xts()
...
kof_mob <- read_csv("https://raw.githubusercontent.com/trendecon/media-content/master/data_examples/article_3_a_mobility_kof.csv") %>%
  ts_xts()
...
article_2 <- read_csv("https://raw.githubusercontent.com/trendecon/media-content/master/data_examples/article_2.csv") %>%
  ts_xts()
